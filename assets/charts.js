(function () {
  ...
  function getDefaultData() {
    return {
      holdings: [],
      valuation: { pe3: null, pe5: null, pe10: null, vix: null },
      trendData: [],
  ...
  var cloudFundData = null;  // 云端 data/fund-data.json 的内容（基金净值）
  ...
  // === Smart Paste: 解析基金App详情页文本，自动填充表单 ===
  var SMART_RULES = [
    { key: 'amount', label: '持有金额', re: /(?:持有金额|持有市值|持仓金额|持有资产)\s*[¥￥:]?\s*([+-]?[\d,，]+\.?\d*)/ },
    { key: 'shares', label: '持有份额', re: /(?:持有份额|持仓份额|持有份数|份额)\s*[:：]?\s*([+-]?[\d,，]+\.?\d*)/ },
    { key: 'costPrice', label: '持仓成本', re: /(?:持仓成本|成本单价|持仓单价|买入成本|成本价)\s*[¥￥:]?\s*([+-]?[\d,，]+\.?\d*)/ },
    { key: 'todayProfit', label: '当日收益', re: /(?:当日收益|今日收益|当日盈亏|今日盈亏)\s*[¥￥:]?\s*([+-]?[\d,，]+\.?\d*)/ },
    { key: 'yesterdayProfit', label: '昨日收益', re: /(?:昨日收益|昨日盈亏)\s*[¥￥:]?\s*([+-]?[\d,，]+\.?\d*)/ },
    { key: 'days', label: '持有天数', re: /(?:持有天数|持仓天数)\s*[:：]?\s*([+-]?\d+)/ }
  ];

  var FUND_NAME_HINT = /(ETF|LOF|联接|指数|混合|债券|股票|货币|精选|成长|价值|QDII|量化|灵活配置|天弘|易方达|广发|华夏|南方|国泰|博时|富国|嘉实|招商|汇添富|中欧|兴全|工银|建信|鹏华|银华|景顺)/;
  ...
  window.smartParse = function () {
    var text = document.getElementById('fSmartPaste').value;
    if (!text.trim()) {
      showToast('请先粘贴基金App的文本');
      return;
    }

    var found = {};
    SMART_RULES.forEach(function (rule) {
      var m = text.match(rule.re);
      if (m) {
        var v = cleanNum(m[1]);
        if (!isNaN(v)) found[rule.key] = { value: v, label: rule.label };
      }
    });

    // 基金名称：找含基金特征词、且不含指标关键词的行
    var fundName = null;
    text.split(/[\n\r]+/).forEach(function (line) {
      if (fundName) return;
      var l = line.trim();
      if (l.length >= 4 && l.length <= 45 && FUND_NAME_HINT.test(l) &&
          !/(持有|份额|占比|收益|成本|天数|涨幅|排名|盈亏|指数涨|板块|走势|弹幕|持仓)/.test(l)) {
        fundName = l.replace(/\s+/g, ' ');
      }
    });
    // 基金代码（6位数字）
    var codeMatch = text.match(/(?:^|[^\d])(\d{6})(?:[^\d]|$)/);
    if (fundName && codeMatch && fundName.indexOf(codeMatch[1]) < 0) {
      fundName = fundName + ' (' + codeMatch[1] + ')';
    }

    // 填充表单
    if (found.amount) document.getElementById('fAmount').value = found.amount.value.toFixed(2);
    if (found.shares) document.getElementById('fShares').value = found.shares.value;
    if (found.costPrice) document.getElementById('fCostPrice').value = found.costPrice.value.toFixed(4);
    if (found.todayProfit) document.getElementById('fProfit').value = found.todayProfit.value.toFixed(2);
    if (found.yesterdayProfit) document.getElementById('fYesterdayProfit').value = found.yesterdayProfit.value.toFixed(2);
    if (found.days && found.days.value >= 0) {
      var d = new Date(Date.now() - found.days.value * 86400000);
      document.getElementById('fStartDate').value = d.toISOString().slice(0, 10);
    }
    if (fundName && !document.getElementById('fName').value) {
      document.getElementById('fName').value = fundName;
    }

    var names = [];
    if (fundName) names.push('名称');
    SMART_RULES.forEach(function (rule) { if (found[rule.key]) names.push(rule.label); });
    if (names.length === 0) {
      showToast('未识别到有效数据，请检查粘贴的文本');
    } else {
      showToast('已识别 ' + names.length + ' 项：' + names.join('、') + '，请核对后保存');
    }
  };
  ...
  window.openAddModal = function () {
    ...
    document.getElementById('fSmartPaste').value = '';
    document.getElementById('fCategory1').value = '股票';
    updateCategory2Options();
    document.getElementById('fName').value = '';
    document.getElementById('fFundCode').value = '';
    ...
    document.getElementById('fYesterdayProfit').value = '';
  ...
  window.editHolding = function (id) {
    ...
    document.getElementById('fFundCode').value = h.fundCode || '';
    document.getElementById('fAmount').value = h.amount;
    document.getElementById('fShares').value = h.shares != null ? h.shares : '';
    document.getElementById('fCostPrice').value = h.costPrice != null ? h.costPrice : '';
    document.getElementById('fStartDate').value = h.startDate || '';
    document.getElementById('fProfit').value = h.todayProfit;
    document.getElementById('fYesterdayProfit').value = h.yesterdayProfit != null ? h.yesterdayProfit : '';
  ...
  window.saveHolding = function () {
    ...
    var fundCodeRaw = document.getElementById('fFundCode').value.trim();
    var fundCode = /^\d{6}$/.test(fundCodeRaw) ? fundCodeRaw : null;
    ...
    var startDate = document.getElementById('fStartDate').value || null;
    var profit = parseFloat(document.getElementById('fProfit').value) || 0;
    var yesterdayProfit = parseFloat(document.getElementById('fYesterdayProfit').value);
    ...
    if (editId) {
      ...
      if (h) {
        ...
        h.fundCode = fundCode;
        h.amount = amount;
        h.shares = isNaN(shares) ? null : shares;
        h.costPrice = isNaN(costPrice) ? null : costPrice;
        h.startDate = startDate;
        h.todayProfit = profit;
        h.yesterdayProfit = isNaN(yesterdayProfit) ? null : yesterdayProfit;
      ...
      data.holdings.push({
        ...
        startDate: startDate,
        todayProfit: profit,
        yesterdayProfit: isNaN(yesterdayProfit) ? null : yesterdayProfit,
        todayChange: change,
        todayAction: action
      });
      showToast('已添加');
  ...
  function getCloudFund(code) {
    if (!code || !cloudFundData || !cloudFundData.funds) return null;
    var f = cloudFundData.funds[code];
    return f && f.ok ? f : null;
  }

  // 自动同步：净值日变化时（新净值公布），自动更新持仓涨跌幅与当日收益
  function applyFundNav(h, f) {
    var pct = f.change_pct;
    h.todayChange = pct;
    h.todayProfit = pct > -100 ? h.amount - h.amount / (1 + pct / 100) : 0;
    h.lastNavDate = f.date;
  }

  function autoSyncFunds() {
    var note = document.getElementById('navDateNote');
    var latestDate = '';
    data.holdings.forEach(function (h) {
      var f = getCloudFund(h.fundCode);
      if (f && f.date > latestDate) latestDate = f.date;
    });
    if (note) {
      note.textContent = latestDate ? '　基金净值日 ' + latestDate + '（QDII T+1~T+2 公布，自动同步）' : '';
    }

    var updated = [];
    data.holdings.forEach(function (h) {
      var f = getCloudFund(h.fundCode);
      if (!f || f.change_pct == null) return;
      if (h.lastNavDate !== f.date) {
        applyFundNav(h, f);
        updated.push(h.name);
      }
    });
    if (updated.length > 0) {
      saveData();
      showToast('已按最新净值自动更新 ' + updated.length + ' 只基金：' + updated.join('、'));
    }
  }

  // 点击「同步」：手动强制按云端净值刷新当日收益
  window.syncFundProfit = function (id) {
    var h = data.holdings.find(function (x) { return x.id === id; });
    var f = h && getCloudFund(h.fundCode);
    if (!h || !f || f.change_pct == null) return;
    applyFundNav(h, f);
    saveData();
    renderAll();
    showToast('已按净值 ' + (f.change_pct > 0 ? '+' : '') + f.change_pct + '% (' + f.date + ') 同步当日收益');
  };

  function calcHoldingProfit(h) {
    if (h.shares > 0 && h.costPrice > 0) {
      return h.amount - h.shares * h.costPrice;
    }
    return null;
  }

  function calcHoldingDays(h) {
    if (!h.startDate) return null;
    var start = new Date(h.startDate + 'T00:00:00');
    if (isNaN(start.getTime())) return null;
    return Math.floor((Date.now() - start.getTime()) / 86400000);
  }

  function signedMoney(v, withYen) {
    var sign = v > 0 ? '+' : '';
    return sign + (withYen ? '¥' : '') + Math.abs(v).toFixed(2);
  }
  ...
  function renderHoldingsTable() {
    ...
    if (data.holdings.length === 0) {
      ...
      tr.innerHTML = '<td colspan="12"><div class="empty-state"><p>暂无持仓记录，点击「添加持仓」开始记录</p></div></td>';
      ...
      data.holdings.forEach(function (h) {
        ...
        var holdingProfit = calcHoldingProfit(h);
        var profitRate = holdingProfit != null && h.shares > 0 && h.costPrice > 0
          ? holdingProfit / (h.shares * h.costPrice) * 100 : null;
        var ratio = totalAmount > 0 ? h.amount / totalAmount * 100 : 0;
        var days = calcHoldingDays(h);

        var dayProfitClass = h.todayProfit > 0 ? 'gain' : (h.todayProfit < 0 ? 'loss' : 'neutral');
        var yesterdayProfit = h.yesterdayProfit;
        var yProfitClass = yesterdayProfit > 0 ? 'gain' : (yesterdayProfit < 0 ? 'loss' : 'neutral');
        var holdProfitClass = holdingProfit > 0 ? 'gain' : (holdingProfit < 0 ? 'loss' : 'neutral');

        // 云端基金净值：显示最新净值日涨跌幅，可一键同步为当日收益
        var cloudFund = getCloudFund(h.fundCode);
        var autoNavHtml = '';
        if (cloudFund && cloudFund.change_pct != null) {
          var navClass = cloudFund.change_pct > 0 ? 'gain' : (cloudFund.change_pct < 0 ? 'loss' : 'neutral');
          autoNavHtml = '<div style="font-size:.7rem;opacity:.85;margin-top:2px">' +
            '<span class="' + navClass + '">净值 ' + (cloudFund.change_pct > 0 ? '+' : '') + cloudFund.change_pct + '% · ' + cloudFund.date + '</span>' +
            ' <a href="javascript:void(0)" onclick="syncFundProfit(' + h.id + ')" style="color:var(--accent);text-decoration:underline">同步</a></div>';
        } else if (h.fundCode) {
          autoNavHtml = '<div style="font-size:.7rem;opacity:.55;margin-top:2px">代码 ' + esc(h.fundCode) + ' 未在云端列表</div>';
        }

        tr.innerHTML =
          '<td><span class="category-tag ' + tagClass + '">' + esc(h.category1) + '</span></td>' +
          '<td style="font-weight:500">' + esc(h.name) + '<div style="font-size:.72rem;opacity:.6;font-weight:400">' + esc(h.category2 || '') + '</div></td>' +
          '<td class="td-amount">' + formatMoney(h.amount) + '</td>' +
          '<td class="td-amount">' + (h.shares != null ? h.shares.toLocaleString('zh-CN', { maximumFractionDigits: 2 }) : '--') + '</td>' +
          '<td class="td-change">' + ratio.toFixed(2) + '%</td>' +
          '<td class="td-profit ' + holdProfitClass + '">' + (holdingProfit != null ? signedMoney(holdingProfit, true) : '--') + '</td>' +
          '<td class="td-change ' + holdProfitClass + '">' + (profitRate != null ? (profitRate > 0 ? '+' : '') + profitRate.toFixed(2) + '%' : '--') + '</td>' +
          '<td class="td-amount">' + (h.costPrice != null ? h.costPrice.toFixed(4) : '--') + '</td>' +
          '<td class="td-profit ' + dayProfitClass + '">' + signedMoney(h.todayProfit, true) + autoNavHtml + '</td>' +
          '<td class="td-profit ' + yProfitClass + '">' + (yesterdayProfit != null ? signedMoney(yesterdayProfit, true) : '--') + '</td>' +
          '<td class="td-amount">' + (days != null ? days : '--') + '</td>' +
    ...
    var hasHoldingProfit = false;
    data.holdings.forEach(function (h) {
      var p = calcHoldingProfit(h);
      if (p != null) { totalHoldingProfit += p; hasHoldingProfit = true; }
    });
    var totalCost = 0;
    data.holdings.forEach(function (h) {
      if (h.shares > 0 && h.costPrice > 0) totalCost += h.shares * h.costPrice;
    });
    var totalProfitRate = totalCost > 0 ? totalHoldingProfit / totalCost * 100 : null;
    var totalYesterday = 0;
    var hasYesterday = false;
    data.holdings.forEach(function (h) {
      if (h.yesterdayProfit != null) { totalYesterday += h.yesterdayProfit; hasYesterday = true; }
    });
    var totalShares = 0;
    var hasShares = false;
    data.holdings.forEach(function (h) {
      if (h.shares != null) { totalShares += h.shares; hasShares = true; }
    });

    document.getElementById('totalAmount').textContent = '¥' + totalAmount.toFixed(2);
    document.getElementById('totalShares').textContent = hasShares ? totalShares.toLocaleString('zh-CN', { maximumFractionDigits: 2 }) : '';
    document.getElementById('totalRatio').textContent = '100%';

    var holdEl = document.getElementById('totalHoldingProfit');
    holdEl.className = 'td-profit ' + (totalHoldingProfit > 0 ? 'gain' : (totalHoldingProfit < 0 ? 'loss' : 'neutral'));
    holdEl.textContent = hasHoldingProfit ? signedMoney(totalHoldingProfit, true) : '';
    var rateEl = document.getElementById('totalHoldingProfitRate');
    rateEl.className = 'td-change ' + (totalProfitRate > 0 ? 'gain' : (totalProfitRate < 0 ? 'loss' : 'neutral'));
    rateEl.textContent = totalProfitRate != null ? (totalProfitRate > 0 ? '+' : '') + totalProfitRate.toFixed(2) + '%' : '';
    ...
    document.getElementById('totalProfit').textContent = signedMoney(totalProfit, true);

    var yEl = document.getElementById('totalYesterdayProfit');
    yEl.className = 'td-profit ' + (totalYesterday > 0 ? 'gain' : (totalYesterday < 0 ? 'loss' : 'neutral'));
    yEl.textContent = hasYesterday ? signedMoney(totalYesterday, true) : '';
  ...
  window.refreshCloudValuation = function () {
    ...
    fetch('./data/fund-data.json', { cache: 'no-store' })
      .then(function (r) {
        if (!r.ok) throw new Error('HTTP ' + r.status);
        return r.json();
      })
  ...
  // 返回单个指标的显示对象 {percentile, value, info, asOf, note, mode}
  // PE 窗口卡（pe3/pe5/pe10）走 windows 结构；VIX 走原值+分位
  function resolveIndicator(item) {
    if (item.win) {
      // PE 历史窗口分位，云端优先
      if (cloudValuation && cloudValuation.indicators) {
        var pe = cloudValuation.indicators.pe;
        if (pe && pe.ok && pe.windows && pe.windows[item.win] && pe.windows[item.win].percentile != null) {
          var w = pe.windows[item.win];
          return {
            percentile: w.percentile,
            info: '当前 PE ' + pe.value + ' · 均值 ' + w.mean,
            note: pe.note || null,
            mode: 'cloud'
          };
        }
      }
      var mv = data.valuation[item.manualKey];
      if (mv != null && !isNaN(mv)) {
        return { percentile: mv, info: null, note: null, mode: 'manual' };
      }
      return null;
    }
    // VIX：云端优先
    if (cloudValuation && cloudValuation.indicators) {
      var ci = cloudValuation.indicators[item.key];
    ...
    var v = data.valuation[item.key];
    if (v != null && !isNaN(v)) {
      return { value: v, percentile: null, asOf: null, note: null, mode: 'manual' };
    }
    return null;
  }
  ...
  function renderValuation() {
    var items = [
      { key: 'pe3', win: 'y3', manualKey: 'pe3', valId: 'gaugePE3', statusId: 'gaugePE3Status', dateId: 'gaugePE3Date' },
      { key: 'pe5', win: 'y5', manualKey: 'pe5', valId: 'gaugePE5', statusId: 'gaugePE5Status', dateId: 'gaugePE5Date' },
      { key: 'pe10', win: 'y10', manualKey: 'pe10', valId: 'gaugePE10', statusId: 'gaugePE10Status', dateId: 'gaugePE10Date' },
      { key: 'vix', valId: 'gaugeVIX', statusId: 'gaugeVIXStatus', dateId: 'gaugeVIXDate' }
    ];
    ...
    items.forEach(function (item) {
      ...
      if (ind.percentile != null) {
        valEl.textContent = ind.percentile.toFixed(1) + '%';
      ...
      var dateParts = [];
      if (ind.info) dateParts.push(ind.info);
      if (ind.asOf) dateParts.push('数据日期 ' + ind.asOf);
      if (item.key === 'vix' && ind.percentile != null) dateParts.push('10年分位 ' + ind.percentile.toFixed(1) + '%');
  ...
  window.openValuationModal = function () {
    document.getElementById('vPE3').value = data.valuation.pe3 != null ? data.valuation.pe3 : '';
    document.getElementById('vPE5').value = data.valuation.pe5 != null ? data.valuation.pe5 : '';
    document.getElementById('vPE10').value = data.valuation.pe10 != null ? data.valuation.pe10 : '';
  ...
  window.saveValuation = function () {
    data.valuation.pe3 = parseFloat(document.getElementById('vPE3').value);
    data.valuation.pe5 = parseFloat(document.getElementById('vPE5').value);
    data.valuation.pe10 = parseFloat(document.getElementById('vPE10').value);
    data.valuation.vix = parseFloat(document.getElementById('vVIX').value);
    ['pe3', 'pe5', 'pe10', 'vix'].forEach(function (k) {
  ...
  window.loadSampleData = function () {
    ...
    data.holdings = [
      { id: 1, category1: '股票', category2: '美股', name: '南方纳斯达克100指数发起(QDII)', amount: 109790.59, shares: 56214.32, costPrice: 1.8856, startDate: '2026-01-15', todayProfit: 520.35, yesterdayProfit: 466.62, todayChange: 0.47, todayAction: '无' },
      { id: 2, category1: '股票', category2: '美股', name: '广发纳指100ETF联接(QDII)人民币F', fundCode: '021778', amount: 45230.18, shares: 19126.54, costPrice: 2.2110, startDate: '2026-02-03', todayProfit: 198.66, yesterdayProfit: 176.31, todayChange: 0.44, todayAction: '无' },
      { id: 3, category1: '股票', category2: '美股', name: '华夏纳斯达克100ETF联接A', amount: 23450.00, shares: 10134.88, costPrice: 2.2296, startDate: '2026-02-20', todayProfit: 105.20, yesterdayProfit: 88.45, todayChange: 0.45, todayAction: '无' },
      { id: 4, category1: '股票', category2: 'A股', name: '红利低波50ETF', amount: 28900.00, shares: 15824.04, costPrice: 1.7642, startDate: '2026-03-10', todayProfit: 0, yesterdayProfit: 42.10, todayChange: 0, todayAction: '无' },
      { id: 5, category1: '股票', category2: 'A股', name: '现金添富ETF', amount: 15600.00, shares: 8210.53, costPrice: 1.8745, startDate: '2026-04-01', todayProfit: 0, yesterdayProfit: -15.30, todayChange: 0, todayAction: '无' },
      { id: 6, category1: '股票', category2: '全球', name: '易方达全球成长精选', amount: 20136.82, shares: 12418.66, costPrice: 1.6325, startDate: '2026-03-05', todayProfit: -38.21, yesterdayProfit: -12.66, todayChange: -0.19, todayAction: '无' },
      { id: 7, category1: '债券', category2: '中长债', name: '南方中债7-10年国开行债券指数A', amount: 149985.00, shares: 141508.49, costPrice: 1.0422, startDate: '2025-12-18', todayProfit: 0, yesterdayProfit: 18.60, todayChange: 0, todayAction: '无' },
      { id: 8, category1: '债券', category2: '短债', name: '上银慧添利90天持有期债券A', amount: 49663.00, shares: 49215.84, costPrice: 1.0038, startDate: '2026-01-28', todayProfit: 0, yesterdayProfit: 8.20, todayChange: 0, todayAction: '无' },
      { id: 9, category1: '黄金', category2: '黄金ETF联接', name: '国泰黄金ETF联接A', amount: 106337.00, shares: 78767.41, costPrice: 1.2540, startDate: '2025-11-06', todayProfit: -316.51, yesterdayProfit: 204.80, todayChange: -0.25, todayAction: '无' },
      { id: 10, category1: '现金', category2: '货币基金', name: '朝朝宝', amount: 21669.00, shares: 21669.00, costPrice: 1.0000, startDate: '2026-05-12', todayProfit: 0, yesterdayProfit: 2.86, todayChange: 0, todayAction: '无' }
    ];
    ...
    // 手动估值作为云端数据不可用时的兜底
    data.valuation = { pe3: 2.6, pe5: 40.5, pe10: 72.6, vix: 14.25 };
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // 加载云端基金净值数据，并自动同步到持仓（失败静默，不影响其他功能）
    fetch('./data/fund-data.json', { cache: 'no-store' })
      .then(function (r) {
        if (!r.ok) throw new Error('HTTP ' + r.status);
        return r.json();
